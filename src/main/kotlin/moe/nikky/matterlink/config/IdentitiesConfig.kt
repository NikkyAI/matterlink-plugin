...
import moe.nikky.matterlink.getList
...
object IdentitiesConfig {
    ...
    fun loadFile() {
            ...
            default.forEach { (uuid, userMap), uuidComment ->
                ...
                if (jsonUserMap is JsonObject) {
                    userMap.forEach { platform, (user, comment) ->
                        logger.finer("loading uuid: $uuid platform: $platform user: $user")
                        val element = MarshallerImpl.getFallback().serialize(user)
                        jsonUserMap.putDefault(platform, element, comment.takeUnless { it.isBlank() })
                    }
                    this[uuid] = jsonUserMap
                } else {
                    logger.severe("cannot parse uuid: $uuid , value: '$jsonUserMap' as Map, skipping")
                }
            }
        }
        ...
        jsonObject = try {
            ...
            logger.severe("cannot find config: $configFile .. creating sample permissions mapping")
    ...
    fun getUUID(platform: String, userid: String): UUID? {
        return idents.entries.firstOrNull { (uuid, usermap) ->
            ...
            try {
                ...
                logger.severe("cannot parse UUID: $it")
