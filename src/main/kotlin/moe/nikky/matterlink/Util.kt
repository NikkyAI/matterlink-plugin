...
val jsonNonstrict = Json(JsonConfiguration.Stable.copy(ignoreUnknownKeys = true, encodeDefaults = false))
...
fun <T : Any> JsonObject.getOrDefault(key: String, default: T, comment: String? = null): T {
    logger.fine("type: ${default.javaClass.name} key: $key json: >>>${this.getObject(key)?.toJson()}<<< default: $default")
...
//
//inline fun <reified I : Any, reified T : Any> Jankson.Builder.registerPrimitiveTypeAdapter(noinline adapter: (I) -> T) =
//    this.registerDeserializer(I::class.java, T::class.java, object: DeserializerFunction<I, T> {
//        override fun apply(a: I, m: Marshaller?): T {
//
//        }
//    })
...
inline fun <reified T : Any> JsonObject.getList(key: String): List<T>? {
    return this[key]?.let { array ->
        when (array) {
            is JsonArray -> {
                array.indices.map { i ->
                    array.get(T::class.java, i) ?: throw NullPointerException("cannot parse ${array[i]}")
                }
            }
            else -> null
        }
